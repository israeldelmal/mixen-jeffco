...
@section('meta')
<meta name="description" content="{!! strip_tags(trim(substr($article->content, 0, 200))) !!}...">
<meta name="author" content="Mixen: Boosting Brands">
@endsection

@section('title', $article->title . ' | JeffCo: Soluciones de Optimización')
...
<main id="blog">
	...
	<section>
		<article data-aos="fade-down" data-aos-duration="500">
			<h1 data-aos="fade-down" data-aos-duration="500">{{ $article->title }}</h1>
			<figure>
				<img src="{{ asset('assets/images/articulos/' . $article->image) }}" alt="Imagen de {{ $article->title }}">
				<figcaption>
					<ul>
			...
			<section>
				{!! $article->content !!}
				<a href="{{ url('/blog') }}">Ver todos <i class="fas fa-arrow-right"></i></a>
			</section>
		</article>
	</section>
</main>
